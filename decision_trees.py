...
def DT_train_binary(X,Y,max_depth):
    ...
    DT = DT_train_helper(DATA_SET, REMAINING_FEATURES)
    return DT          # returns decision tree
...
def DT_train_helper(data, remaining_features):
    ...
    if num_yes_no_label[0] == 0 or num_yes_no_label[1] == 0:     # if unambiguous
        ...
        highest_IG_feature_remaining = IG_sorted[0][0]
        no_subset = select_subset(data, highest_IG_feature_remaining, no)
        yes_subset = select_subset(data, highest_IG_feature_remaining, yes)
        remaining_features[highest_IG_feature_remaining] = 0
        left = DT_train_helper(no_subset, remaining_features)
        right = DT_train_helper(yes_subset, remaining_features)
        return NODE(highest_IG_feature_remaining, left, right)
    
def DT_test_binary(X,Y,DT):
    Y_vert = Y.reshape(Y.shape[0],-1)        # package labels with feature as
    DATA_SET = np.hstack((X, Y_vert))               # last column in 2D array
    for test_point in DATA_SET:
        DT_test_binary_helper(DT, test_point, DATA_SET)
 
        
def DT_test_binary_helper(tree, test_point, data):
    num_rows, num_cols = data.shape
    label = num_cols - 1
    left = tree[1][0]
    if num_rows == 1:
        ...
        feature = tree[0]
        if data[feature][label] == no:
            return DT_test_binary_helper(left, test_point, data)
