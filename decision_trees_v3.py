...
def calculate_IG(data, feature): 
    ...
    entropy_whole_set = calculate_Entropy(data, labels)
    yes_subset = select_subset(data, feature, yes)
    no_subset = select_subset(data, feature, no)
    entropy_yes_subset = calculate_Entropy(yes_subset, feature)
    entropy_no_subset = calculate_Entropy(no_subset, feature)
    return entropy_whole_set - (probability_no * entropy_no_subset + 
                                probability_yes * entropy_yes_subset)

def calculate_Entropy(data, col): #col is column to calulate on
    num_rows, num_cols = data.shape
    i = 0
    num_yes_no = count_YN(data, col)
    probability_yes = num_yes_no[0] / num_rows
    probability_no = num_yes_no[1] / num_rows
